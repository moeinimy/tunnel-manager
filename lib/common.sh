...
ensure_dirs() {
    local d
    for d in "$TM_CONFIG_DIR" "$TM_TUNNELS_DIR" "$TM_PAQET_DIR" "$TM_BACKHAUL_DIR" "$TM_BACKPACK_DIR" "$TM_HYSTERIA_DIR" "$TM_REALITY_DIR" "$TM_RATHOLE_DIR" "$TM_GOST_DIR" "$TM_FRP_DIR" \
